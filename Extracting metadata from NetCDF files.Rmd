...
This section applies some steps of the generalize function. Those steps are taken from the source code of the generalize function. I am attempting to understand how the function works in order to decide what generalozation method to use and what the value is.

As an example the Track object of File1 is used.

#### Questions regarding the Track class
What does the endtime mean?
"@endtime to store the end time when having
generalised line geometries with one value per attribute for a set of points (otherwise, de-
faults to the time defined in @time)" ??

By default, distance, duration, speed
and direction are computed as the connections data.??




#### Checking the first section of the timeInterval
##### Using time interval
It looks like when using the timeInterval the generalized track has as many lines as defined in the time interval.

t@time has the class "xts" "zoo"
index(t@time) has the class "POSIXct" "POSIXt"
typeof(t@time) is integer
typeof(index(t@time)) is double

the function cut categorizes the values of a continuous variable in different levels of a factor.
The time the Track object has is divided into the number specified by the argument timeInterval.

segmentLengths counts how many incidents each factor of the result of the cut function has.

```{r}
t = missionTrack_File1
# time interval is 60                                                   seconds
timeInterval = 60
# Applying the steps
    # The function index belongs to the zoo package
    origin = index(t@time)
...
t = missionTrack_File1
distance = 100
# It calculates the cumulative distance starting from the first point to each point
origin = c(0, cumsum(t@connections$distance))
# floor takes a single numeric argument x and returns a numeric vector containing the largest integers not greater than the corresponding elements of x.
# floor() function returns the largest integer that is smaller than or equal to value passed to it as argument(i.e : rounds downs to the nearest integer).
cut = floor(origin / distance)
# This returns how many times each number in the cut object there is
...
### Plotting the track before and after the generalization
```{r}
# plot(mission_Track_File3)
plot(mission_Track_File1_generalized_distance, main = "The generalized track")
plot(missionTrack_File1, main = "The whole track")
```
